...
class CreditDetailsService:
    ...
    def enrich_credit_details(self, reporte_df, sc04_df, fnz001_df):
        ...
        if sc04_df.empty and fnz001_df.empty:
            print(
                "⚠️ No se encontraron archivos SC04 ni de Desembolsos. Se omiten los detalles del crédito."
            )
            return reporte_df
        ...
        if not sc04_df.empty:
            ...
            sc04_df["Factura_Venta"] = sc04_df["Factura_Venta"].apply(
                transformar_factura
            )
            ...
            sc04_df["Valor_Cuota"] = pd.to_numeric(
                sc04_df["Valor_Cuota"], errors="coerce"
            )
            sc04_df["Total_Cuotas"] = pd.to_numeric(
                sc04_df["Total_Cuotas"], errors="coerce"
            )
            sc04_df["Pago_Inicial"] = pd.to_numeric(
                sc04_df["Pago_Inicial"], errors="coerce"
            )
            sc04_df["Valor_Desembolso"] = (
                sc04_df["Valor_Cuota"] * sc04_df["Total_Cuotas"]
            ) + sc04_df["Pago_Inicial"]
            ...
            # --- CORRECCIÓN pandas 3.0: castear a object para aceptar tipos mixtos ---
            # Si las columnas ya existen con dtype 'str' (e.g. desde base anterior),
            # asignar valores numéricos falla con "Invalid value for dtype 'str'".
            ...
            reporte_df.loc[mask_arp, "Total_Cuotas"] = reporte_df.loc[
                mask_arp, "Factura_Venta"
            ].map(mapa_cuotas_arp)
            reporte_df.loc[mask_arp, "Valor_Cuota"] = reporte_df.loc[
                mask_arp, "Factura_Venta"
            ].map(mapa_valor_arp)
            reporte_df.loc[mask_arp, "Valor_Desembolso"] = reporte_df.loc[
                mask_arp, "Factura_Venta"
            ].map(mapa_desembolso_arp)
        ...
        if not fnz001_df.empty:
            ...
            # --- CORRECCIÓN pandas 3.0: misma protección que ARPESOD ---
            for col in ["Total_Cuotas", "Valor_Cuota", "Valor_Desembolso"]:
                if col in reporte_df.columns:
                    reporte_df[col] = reporte_df[col].astype(object)
            reporte_df.loc[mask_fns, "Total_Cuotas"] = reporte_df.loc[
                mask_fns, "Credito"
            ].map(mapa_cuotas_fns)
            reporte_df.loc[mask_fns, "Valor_Cuota"] = reporte_df.loc[
                mask_fns, "Credito"
            ].map(mapa_valor_fns)
            reporte_df.loc[mask_fns, "Valor_Desembolso"] = reporte_df.loc[
                mask_fns, "Credito"
            ].map(mapa_desembolso_fns)
    ...
    def process_vencimientos_data(self, vencimientos_df):
        ...
        df["Fecha_Cuota_Vigente"] = pd.to_datetime(
            df["Fecha_Cuota_Vigente"], errors="coerce"
        )
        df["Valor_Cuota_Vigente"] = pd.to_numeric(
            df["Valor_Cuota_Vigente"], errors="coerce"
        )
        df.dropna(subset=["Credito", "Fecha_Cuota_Vigente"], inplace=True)
        today = pd.Timestamp.now().normalize()
        resumen_creditos = pd.DataFrame(
            df["Credito"].unique(), columns=["Credito"]
        ).set_index("Credito")

        # --- Añadir la columna 'Celular' al resumen ---
        if "Celular" in df.columns:
            mapa_celular = df.groupby("Credito")["Celular"].first()
            resumen_creditos["Celular"] = resumen_creditos.index.map(mapa_celular)

        # --- Inicializar Valor_Vencido con 0 para todos los créditos ---
        resumen_creditos["Valor_Vencido"] = 0

        # --- Cálculos de Atraso ---
        df_atrasados = df[df["Fecha_Cuota_Vigente"] < today].copy()

        creditos_con_negativos = pd.DataFrame()
        if not df_atrasados.empty:
            cuotas_negativas_df = df_atrasados[df_atrasados["Valor_Cuota_Vigente"] < 0]
            if not cuotas_negativas_df.empty:
                print(
                    f"   - ⚠️ Se encontraron {len(cuotas_negativas_df)} cuotas con valores negativos."
                )
                creditos_con_negativos = (
                    cuotas_negativas_df[["Credito", "Cedula_Cliente"]]
                    .drop_duplicates()
                    .reset_index(drop=True)
                )
                creditos_con_negativos["Observacion"] = (
                    "Valor negativo en Informe de Vencimientos"
                )

            # Calcular suma de valores vencidos para cada crédito
            mapa_valor_vencido = df_atrasados.groupby("Credito")[
                "Valor_Cuota_Vigente"
            ].sum()
            # Actualizar los valores en el resumen (solo para créditos con mora)
            resumen_creditos["Valor_Vencido"] = resumen_creditos["Valor_Vencido"].add(
                mapa_valor_vencido, fill_value=0
            )

            # Resto de cálculos de atraso
            idx_primera_mora = df_atrasados.groupby("Credito")[
                "Fecha_Cuota_Vigente"
            ].idxmin()
            mapa_primera_mora = df.loc[idx_primera_mora].set_index("Credito")

            resumen_creditos["Fecha_Cuota_Atraso"] = resumen_creditos.index.map(
                mapa_primera_mora["Fecha_Cuota_Vigente"]
            )
            resumen_creditos["Primera_Cuota_Mora"] = resumen_creditos.index.map(
                mapa_primera_mora["Cuota_Vigente"]
            )
            resumen_creditos["Valor_Cuota_Atraso"] = resumen_creditos.index.map(
                mapa_primera_mora["Valor_Cuota_Vigente"]
            )

        # --- Cálculos de Vigencia ---
        df_vigentes = df[
            (df["Fecha_Cuota_Vigente"].dt.year == today.year)
            & (df["Fecha_Cuota_Vigente"].dt.month == today.month)
        ].copy()
        if not df_vigentes.empty:
            idx_ultima_vigente = df_vigentes.groupby("Credito")[
                "Fecha_Cuota_Vigente"
            ].idxmax()
            mapa_vigentes = df.loc[idx_ultima_vigente].set_index("Credito")

            resumen_creditos["Fecha_Cuota_Vigente"] = resumen_creditos.index.map(
                mapa_vigentes["Fecha_Cuota_Vigente"]
            )
            resumen_creditos["Cuota_Vigente"] = resumen_creditos.index.map(
                mapa_vigentes["Cuota_Vigente"]
            )
            resumen_creditos["Valor_Cuota_Vigente"] = resumen_creditos.index.map(
                mapa_vigentes["Valor_Cuota_Vigente"]
            )
        ...
        creditos_anticipados = (
            creditos_con_cuotas_futuras
            - creditos_con_cuotas_este_mes
            - creditos_con_cuotas_pasadas
        )

        if creditos_anticipados:
            print(
                f"   - ✅ Se identificaron {len(creditos_anticipados)} créditos que serán marcados como 'ANTICIPADO'."
            )
            columnas_a_marcar = [
                "Fecha_Cuota_Atraso",
                "Primera_Cuota_Mora",
    ...
    def adjust_arrears_status(self, reporte_df):
        ...
        if "Dias_Atraso" in reporte_df.columns:
            sin_mora_mask = (
                pd.to_numeric(reporte_df["Dias_Atraso"], errors="coerce").fillna(0) == 0
            )

            columnas_mora_a_limpiar = [
                "Fecha_Cuota_Atraso",
                "Primera_Cuota_Mora",
                "Valor_Cuota_Atraso",
                "Valor_Vencido",
