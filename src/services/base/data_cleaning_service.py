...
class DataCleaningService:
    ...
    def clean_email_column(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "Correo" in df.columns:
            ...
            if invalid_count > 0:
                print(
                    f"   - ⚠️ Se encontraron {invalid_count} correos inválidos que serán reemplazados."
                )

            # Reemplazamos los correos inválidos (donde la máscara es False) por NaN
            # --- CORRECCIÓN pandas 3.0: np.nan es float, no se puede asignar a columna str ---
            df["Correo"] = df["Correo"].astype(object)
    ...
    def _obtener_serie_valida(
        self, series: pd.Series, solo_10_digitos: bool = False
    ) -> pd.Series:
        ...
        es_celular_valido = (telefonos_limpios.str.len() == 10) & (
            telefonos_limpios.str.startswith("3")
        )
        es_fijo_nacional_valido = (telefonos_limpios.str.len() == 10) & (
            telefonos_limpios.str.startswith("60")
        )

        # Regla de 7 dígitos (Fijo Local)
        prefijos_fijos_validos = ("2", "4", "5", "6", "7", "8")
        es_fijo_local_valido = (telefonos_limpios.str.len() == 7) & (
            telefonos_limpios.str.startswith(prefijos_fijos_validos)
        )
        ...
        if solo_10_digitos:
            ...
            mask_formato_valido = (
                es_celular_valido | es_fijo_nacional_valido | es_fijo_local_valido
            )

        mask_final_valido = mask_formato_valido & no_son_repetidos
        # --- CORRECCIÓN pandas 3.0: np.nan es float, .where() falla con dtype str ---
        # Convertimos a object antes de usar .where() con np.nan
        telefonos_limpios = telefonos_limpios.astype(object)
    ...
    def unificar_telefonos_codeudores(
        self,
        df: pd.DataFrame,
        col_principal: str,
        col_secundaria: str,
        col_destino: str,
        valor_defecto: str = "SIN CODEUDOR",
        solo_10_digitos: bool = False,
    ) -> pd.DataFrame:
        """
        ...
        if col_principal in df.columns:
            s_principal_valida = self._obtener_serie_valida(
                df[col_principal], solo_10_digitos=solo_10_digitos
            )

        s_secundaria_valida = pd.Series(np.nan, index=df.index)
        if col_secundaria and col_secundaria in df.columns:
            s_secundaria_valida = self._obtener_serie_valida(
                df[col_secundaria], solo_10_digitos=solo_10_digitos
            )

        # 2. Aplicar la lógica de selección
        df[col_destino] = np.where(
            s_principal_valida.notna(), s_principal_valida, s_secundaria_valida
        )
