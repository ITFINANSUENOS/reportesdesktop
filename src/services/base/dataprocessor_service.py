...
class ReportProcessorService:
    ...
    def filter_by_date_range(self, reporte_df, start_date, end_date):
        ...
        df["Fecha_Cuota_Vigente"] = pd.to_datetime(
            df["Fecha_Cuota_Vigente"], format="%d/%m/%Y", errors="coerce"
        )
        ...
        mask = (df["Fecha_Cuota_Vigente"] >= start_date_dt) & (
            df["Fecha_Cuota_Vigente"] <= end_date_dt
        )
        filtered_df = df[mask]
        print(
            f"✅ Filtro aplicado. {len(filtered_df)} registros encontrados en el rango."
        )
        return filtered_df
    ...
    def finalize_report(self, reporte_df, orden_columnas):
        ...
        columnas_a_eliminar = [
            ...
            *[
                ...
                if col.endswith(("_Analisis", "_R03", "_Venc", "_display", "_dup"))
            ],
        ]
        ...
        columnas_restantes = [
            col for col in columnas_actuales if col not in columnas_ordenadas
        ]
    ...
    def _clean_leader_info(self, df):
        ...
        if "Lider_Zona" in df.columns and "Regional_Venta" in df.columns:
            ...
            df["Lider_Zona"] = df["Lider_Zona"].astype(object)
            df.loc[is_numeric_mask, "Lider_Zona"] = np.nan

            mapa_moviles = {}
            if "Movil_Lider" in df.columns:
                mapa_df = df.dropna(
                    subset=["Lider_Zona", "Movil_Lider"]
                ).drop_duplicates(subset=["Lider_Zona"])
                mapa_moviles = pd.Series(
                    mapa_df["Movil_Lider"].values, index=mapa_df["Lider_Zona"]
                ).to_dict()
            ...
            df["Lider_Zona"] = df.groupby("Regional_Venta")["Lider_Zona"].transform(
                fill_with_mode
            )
            if "Movil_Lider" in df.columns:
                df["Movil_Lider"] = df["Lider_Zona"].map(mapa_moviles)
            df["Lider_Zona"] = df["Lider_Zona"].fillna("NO ASIGNADO")
            if "Movil_Lider" in df.columns:
                df["Movil_Lider"] = df["Movil_Lider"].fillna("NO ASIGNADO")
        return df
